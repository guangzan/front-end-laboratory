...
<template>
  <div class="max-w-3/4 mx-auto mt-8">
    ...
    <div class="flex justify-between gap-8 <sm:(flex-col)">
      <div class="relative flex-1 min-h-96 bg-gray-100 border border-gray-200 rounded-md p-4">
        <v-button
        ...
        <label
          for="upload-image"
          class="flex items-center justify-center w-full h-full bg-no-repeat bg-contain bg-center cursor-pointer"
          :style="{ backgroundImage: `url(${imageSrc})` }"
        >
          <UploadIcon class="h-12 w-12 ml-2 text-blue-500" v-show="imageSrc === ''"></UploadIcon>
        </label>
        <input class="opacity-0" id="upload-image" type="file" @change="fileSelected($event)" />
      </div>
      <div class="flex-1 relative" ref="input">
        <textarea
          v-model="base64String"
          @blur="handleBase64AreaBlur"
          class="w-full block bg-gray-100 text-gray-700 p-2 rounded-md border border-gray-200 shadow-md whitespace-pre-wrap font-mono min-h-96"
        ></textarea>
        <v-button
          class="flex align-center absolute right-6 top-4 !px-2 !py-1"
          size="sm"
